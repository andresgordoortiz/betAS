...
getDataset <- function(pathTables=NULL, tool){
  ...
  if (is.null(pathTables)){
    ...
    if (tool == "vast-tools"){
      ...
      stop("rMATS test dataset not available. Please provide pathTables to your rMATS file.")

    } else if (tool == "whippet"){

      stop("Whippet test dataset not available. Please provide pathTables to your whippet files.")
...
alternativeEvents  <- function(filteredList, minPsi, maxPsi){
  ...
  psi                     <- psiTable[,-c(1:6)]
  psiTable$AllGreaterMin  <- rowSums(psi < minPsi) == 0
  psiTable$AllLowerMax    <- rowSums(psi > maxPsi) == 0
